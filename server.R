...
#dname: user selected name of institution
#sector: user selected name of sector
#state: user selected name of state
...
server <- function(input, output, session) {
  ...
  # This is the user's choice of university
  ...
  observeEvent({
    ...
    withProgress(message = 'Please wait',
                 ...
                 value = 0,
                 {
                   ...
                   if (is.null(input$sector) & input$state == "--none--") {
                     updateSelectizeInput(inputId = "dname", choices = institution_choice())
  ...
  # ----. Compare Selectize input part 3
  ...
  output$num_schools_in_sector <- renderText({
    validate(need(input$dname != "--none--", ''))
    
    paste(
      "Percentile rank is shown within all",
      num_schools(),
  ...
  col_stops <- reactive({
    data.frame(q = c(.9), c = c('#3d6c72'))
  })
  
  output$fhii_score_gauge <- renderHighchart({
    validate(need(input$dname != "--none--", 'Please Choose Institution'))
  ...
  pell_enroll_pct <- reactive({
    efa_fsums_pell_compl_profile_equity %>%
      ...
      select(dname,
             year,
             pell_pct_per_inst_year,
             nonpell_pct_per_inst_year) %>%
      drop_na %>%
      group_by(dname) %>%
  ...
  output$race_enroll_pie <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_plotOptions(series = list(showInLegend = TRUE)) %>%
      hc_colors(c(aian, asian, black, hispanic, nhpi, race_unk, two_race, white)) %>%
      hc_tooltip(pointFormat = "{point.percentage:,.2f}%") %>%
      hc_title(
        text = "Race",
        style = list(
  ...
  completion_generic_data <- reactive({
    efa_fsums_pell_compl_profile_equity %>% ungroup() %>%
      select(dname,
             year,
             completion_rate_per_inst_year,
             non_completion_rate_per_inst_year) %>%
      drop_na %>%
      group_by(dname) %>%
      filter(year == max(year)) %>%
      distinct() %>%
  ...
  output$grand_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(completion_generic_data()$value[1])} </center></B></FONT>"
      )
    )
    ...
    completion_generic_data() %>%
      ...
      hc_tooltip(
        formatter =
          JS(
            "function () { return ' <br /> Overall Completion Rate ' + (this.y.toFixed(2)*100) +'%' ;}"
          )
      ) %>%
      hc_labels(items = list(list(
        html = labels,
        style = list(
          fontSize = "16px",
          fontWeight = "bold",
  ...
  output$pell_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(pell_compl_react_data()$value[1])} </center></B></FONT>"
      )
    )
    ...
    pell_compl_react_graph1() %>%
      hc_add_series(
        data = {
          pell_compl_react_data()
        },
        type = "pie",
        hcaes(
          x = pell_compl,
          y = value,
          color = c("#3e6d72", "#ffffff")
        ),
        animation = T,
        size = '100%',
        innerSize = '75%',
  ...
  output$non_pell_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(pell_compl_react_data()$value[2])} </center></B></FONT>"
      )
    )
  ...
  race_compl_react_data <- reactive({
    efa_fsums_pell_compl_profile_equity %>% ungroup() %>%
      select(
        ...
        non_completion_rate_per_inst_year_race
      ) %>%
      filter(dname == input$dname) %>%
      group_by(dname) %>%
      filter(year == max(year)) %>%
  ...
  make_completion_pie = function(race_name, colors, color2) {
    renderHighchart({
      validate(
        need(input$dname != "--none--", 'Please Choose Institution'),
      ...
      race_data <- race_compl_react_data() %>%
        ungroup() %>%
        filter(race == !!race_name &
                 race_compl == "completion_rate_per_inst_year_race")
      
      if (is.na({
        race_compl_react_data() %>% ungroup() %>%
          filter(race == race_name &
                 race_compl == "completion_rate_per_inst_year_race") %>%
          pull(value)
      }))
      ...
      labels <- htmltools::HTML(
        glue::glue(
          "<B><FONT SIZE = +30><center> {scales::percent(race_data$value[9])} </center></B></FONT>"
        )
      )
      ...
      pell_compl_react_graph1() %>%
        ...
        hc_tooltip(formatter = JS("function () { return (this.y*100).toFixed(2) + '%' ;}")) %>%
        hc_labels(items = list(list(
          html = labels,
          style = list(
            fontSize = "16px",
  ...
  render_pie_chart <- function(race, color, value_index) {
    renderHighchart({
      validate(
        need(input$dname != "--none--", 'Please Choose Institution'),
        need(nrow(race_compl_react_data()) != 0, 'No Data Available')
      ...
      labels <- htmltools::HTML(
        glue::glue(
          "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[{value_index}])} </center></B></FONT>"
        )
      )
  ...
  output$white_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[15])} </center></B></FONT>"
      )
    )
  ...
  output$hispanic_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[7])} </center></B></FONT>"
      )
    )
  ...
  output$aian_compl_pie <- renderHighchart({
    ...
    if (is.nan(
      race_compl_react_data() %>% ungroup() %>%
      filter(
        race == "AIAN" & race_compl == "completion_rate_per_inst_year_race"
      ) %>%
      pull(value)
    ))
      return()
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[1])} </center></B></FONT>"
      )
    )
  ...
  output$black_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[5])} </center></B></FONT>"
      )
    )
  ...
  output$asian_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[3])} </center></B></FONT>"
      )
    )
  ...
  output$nhpi_compl_pie <- renderHighchart({
    ...
    if (is.nan(
      race_compl_react_data() %>% ungroup() %>%
      filter(
        race == "NHPI" & race_compl == "completion_rate_per_inst_year_race"
      ) %>%
      pull(value)
    ))
      return()
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[9])} </center></B></FONT>"
      )
    )
  ...
  output$two_or_more_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[13])} </center></B></FONT>"
      )
    )
  ...
  output$race_unknown_compl_pie <- renderHighchart({
    ...
    labels <- htmltools::HTML(
      glue::glue(
        "<B><FONT SIZE = +30><center> {scales::percent(race_compl_react_data()$value[11])} </center></B></FONT>"
      )
    )
  ...
  efa_fsums_pell_compl_profile_equity <- efa_fsums_pell_compl_profile_equity %>%
    ...
    mutate_at(c("race_long"),
              str_replace_all,
              pattern = "AIAN",
              replacement = "American Indian and Alaskan Native") %>%
    mutate_at(c("race_long"),
              str_replace_all,
              pattern = "NHPI",
              replacement = "Native Hawaiian and Pacific Islander") %>%
    mutate(enrollment_pct_per_inst_year_race_2 = enrollment_pct_per_inst_year_race *
             100) %>%
    mutate(instructor_pct_per_inst_year_race_2 = instructor_pct_per_inst_year_race *
             100) %>%
    mutate(pop_pct_per_state_year_race_2 = pop_pct_per_state_year_race *
             100) %>%
    mutate(pop_pct_per_natl_year_race_2 = pop_pct_per_natl_year_race * 100) %>%
    mutate(completers_pct_per_inst_year_race_2 = completers_pct_per_inst_year_race *
             100) %>%
    mutate(completion_rate_per_inst_year_race_2 = completion_rate_per_inst_year_race *
             100)
  ...
  bar_data <- reactive({
    efa_fsums_pell_compl_profile_equity %>%
      ...
      mutate(enrollment_pct_per_inst_year_race_1 = enrollment_pct_per_inst_year_race *
               100) %>%
      mutate(instructor_pct_per_inst_year_race_1 = instructor_pct_per_inst_year_race *
               100) %>%
      mutate(pop_pct_per_state_year_race_1 = pop_pct_per_state_year_race *
               100) %>%
      mutate(pop_pct_per_natl_year_race_1 = pop_pct_per_natl_year_race *
               100) %>%
      mutate(completers_pct_per_inst_year_race_1 = completers_pct_per_inst_year_race *
               100) %>%
      mutate(completion_rate_per_inst_year_race_1 = completion_rate_per_inst_year_race *
               100) %>%
      distinct()
  ...
  averages <- reactive({
    efa_fsums_pell_compl_profile_equity %>%
      ...
      mutate(drilldown = tolower(race_long)) %>%
      mutate(enrollment_pct_per_inst_year_race_1 = enrollment_pct_per_inst_year_race_1 *
               100) %>%
      mutate(instructor_pct_per_inst_year_race_1 = instructor_pct_per_inst_year_race_1 *
               100) %>%
      mutate(pop_pct_per_state_year_race_1 = pop_pct_per_state_year_race_1 *
               100) %>%
      mutate(pop_pct_per_natl_year_race_1 = pop_pct_per_natl_year_race_1 *
               100) %>%
      mutate(completers_pct_per_inst_year_race_1 = completers_pct_per_inst_year_race_1 *
               100) %>%
      mutate(completion_rate_per_inst_year_race_1 = completion_rate_per_inst_year_race_1 *
               100)
  ...
  #Used to reduce long code in the highchart
  ...
  analysis_levels <- c("institution", "state", "nation")
  
  instructor_enrollment = c("Instructor", "Enrollment")
  
  create_list = function(race, df) {
    list(name = race,
         y = {
           bar_data() %>%
             filter(race_long == race) %>%
             pull(df)
         },
         drilldown = race)
  }
  
  #Used to create graph for each level of comparison for given racial demographic
  create_graph_list = function(graph_name, graph_id, level) {
    color <- switch(
      level,
      "institution" = "black",
      "state" = "#3e6d72",
    ...
    data_column <- switch(
      level,
      "institution" = "enrollment_pct_per_inst_year_race_2",
      "state" = "pop_pct_per_state_year_race_2",
      "nation" = "pop_pct_per_natl_year_race_2",
      "Instructor" = "instructor_pct_per_inst_year_race_2",
      "Enrollment" = "enrollment_pct_per_inst_year_race_2",
      "Completion" = "completion_rate_per_inst_year_race_2"
    )
    ...
    list(
      ...
      data = list_parse2(
        efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          filter(race_long == graph_id) %>%
          select("year", data_column)
      )
    )
  }
  ...
  output$enrollment_bar <- renderHighchart({
    ...
    highchart() %>%
      hc_chart(
        type = "column",
        inverted = TRUE,
        events = list(drilldown = JS(fn), drillupall = JS(fn2)),
        marginLeft = 285
      ) %>%
      hc_title(
      ...
      hc_yAxis(labels = list(overflow = "allow", padding = 100)) %>%
      ...
      hc_add_series(
        ...
        color = "#bfdcd7",
        pointPadding = -.2,
        data = lapply(races, create_list, df = "pop_pct_per_natl_year_race_1")
      ) %>%
      hc_add_series(
      ...
      hc_drilldown(
        ...
        series = unlist(lapply(races, function(race) {
          lapply(analysis_levels, function(level) {
            create_graph_list(paste(race, level, "Percent"), race, level)
          })
        }), recursive = FALSE)
      )
  })
  ...
  output$completers_bar <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_subtitle(text = paste("", "Select any race to see the trend over time for that race")) %>%
      hc_plotOptions(column = list(grouping = FALSE, shadow = FALSE)) %>%
      hc_tooltip(
        valueDecimals = 1,
        shared = TRUE,
  ...
  output$instructor_enrollment_bar <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_add_series(
        ...
        color = "#bfdcd7",
        pointPadding = -.2,
        data = lapply(races, create_list, df = "instructor_pct_per_inst_year_race_1")
      ) %>%
      hc_add_series(
      ...
      hc_drilldown(
        ...
        activeAxisLabelStyle = list(text = "does it work?"),
        series = unlist(lapply(races, function(race) {
          lapply(instructor_enrollment, function(level) {
            create_graph_list(paste(race, level, "Percent"), race, level)
          })
        }), recursive = FALSE)
      )
  ...
  #Quang: I'm not sure where the next two output items are used
  ...
  output$enrollment_completion_bar <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_add_series(
        ...
        color = "#bfdcd7",
        pointPadding = -.2,
        data = lapply(races, create_list, df = "completion_rate_per_inst_year_race_1")
      ) %>%
      hc_add_series(
      ...
      hc_drilldown(
        ...
        activeAxisLabelStyle = list(text = "does it work?"),
        series = unlist(lapply(races, function(race) {
          lapply(c("Completion", "Enrollment"), function(level) {
            create_graph_list(paste(race, level, "Percent"), race, level)
          })
        }), recursive = FALSE)
      )
  ...
  output$retention <- renderHighchart({
    highchart() %>%
      ...
      hc_add_series(
        name = "Full Time Retention",
        efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          select(year, ft_retention) %>%
          distinct(),
        "line",
        hcaes(x = year, y = ft_retention)
      ) %>%
      hc_add_series(
        name = "Part Time Retention",
        efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          select(year, pt_retention) %>%
          distinct(),
        "line",
        hcaes(x = year, y = pt_retention)
      ) %>%
      hc_yAxis(
        title = list(text = "Retention Rate"),
        min = 0,
        max = 100,
        tickInterval = 20
      ) %>%
  ...
  output$retention_comp <- renderHighchart({
    validate(need(input$dname_comp != "--none--", '  '))
    
    highchart() %>%
      hc_title(
        text = paste(comp_institution()),
        style = list(
          fontSize = "18px",
          color = "black",
          fontFamily = "Roboto",
  ...
  output$retention_comp2 <- renderHighchart({
    validate(need(input$dname2_comp != "--none--", '  '))
    
    highchart() %>%
      hc_title(
        text = paste(comp_institution2()),
        style = list(
          fontSize = "18px",
          color = "black",
          fontFamily = "Roboto",
  ...
  output$retention_comp3 <- renderHighchart({
    validate(need(input$dname3_comp != "--none--", '  '))
    
    highchart() %>%
      hc_title(
        text = paste(comp_institution3()),
        style = list(
          fontSize = "18px",
          color = "black",
          fontFamily = "Roboto",
  ...
  #Might be able to reduce this down by making a function for the comparison charts
  
  #Graduation comparison
  
  add_series = function(chart, long_name, short_name, df) {
    chart %>% hc_add_series(
      name = long_name,
      efa_fsums_pell_compl_profile_equity %>%
        filter(dname == input$dname) %>%
        filter(race == short_name) %>%
        select(year, df),
      "line",
      hcaes(x = year, y = df)
    )
  ...
  #List of racial demographics
  race_short_long_names = list(
  ...
  graduation_react <- reactive({
    chart = highchart() %>% hc_add_series(
      name = "Overall Graduation Rate",
      efa_fsums_pell_compl_profile_equity %>% ungroup() %>%
        select(
          dname,
          year,
    ...
    lapply(names(race_short_long_names), function(short_name) {
      chart <<- add_series(
        chart,
        race_short_long_names[[short_name]],
        short_name,
        completers_pct_per_inst_year_race
      )
    })
    
    chart %>% hc_yAxis(title = list(text = "Percent"),
                       min = 0,
                       max = 1)
  })
  ...
  output$graduation <- renderHighchart({
    ...
    lapply(names(race_short_long_names), function(short_name) {
      chart <<- chart %>%
        hc_add_series(
          name = race_short_long_names[[short_name]],
          efa_fsums_pell_compl_profile_equity %>%
            filter(dname == input$dname) %>%
            mutate(
              completion_rate_per_inst_year_race = round(
                completion_rate_per_inst_year_race * 100, 0)
            ) %>%
            filter(race == short_name) %>%
            select(year, completion_rate_per_inst_year_race),
          "line",
          hcaes(x = year, y = completion_rate_per_inst_year_race),
          visible = FALSE
        )
    })
    ...
    #Needed to allow user to filter racial demographics 
    chart
  })
      ...
      chart <<- chart %>%
        hc_add_series(
          ...
          efa_fsums_pell_compl_profile_equity %>%
            filter(dname == input$dname) %>%
            mutate(
              enrollment_pct_per_inst_year_race = round(
                enrollment_pct_per_inst_year_race * 100, 0)
            ) %>%
            filter(race == short_name) %>%
            select(year, enrollment_pct_per_inst_year_race),
          "line",
          hcaes(x = year, y = enrollment_pct_per_inst_year_race),
          visible = FALSE
        )
    })
    ...
    highchart() %>%
      ...
      hc_add_series(
        ...
        efa_fsums_pell_compl_profile_equity %>%
          ...
          distinct(),
        "line",
        hcaes(x = year, y = transfer_out_rate)
      ) %>%
      
      hc_legend(enabled = FALSE) %>%
      hc_yAxis(
        title = list(text = "Transfer-Out Rate"),
        min = 0,
        max = 100,
        tickInterval = 20
      ) %>%
  ...
  input_names <- c("dname_comp", "dname2_comp", "dname3_comp")
  comp_institutions <- list(comp_institution, comp_institution2, comp_institution3)
  
  lapply(seq_along(input_names), function(i) {
    output[[paste0("transfer_comp", if (i > 1) i else "")]] <- renderHighchart({
      validate(need(input[[input_names[i]]] != "--none--", '  '))
      
      highchart() %>%
        hc_title(
          text = paste(comp_institutions[[i]]()),
          style = list(
            fontSize = "18px",
            color = "black",
            fontFamily = "Roboto",
        ...
        hc_add_series(
          name = "Transfer-Out Rate",
          efa_fsums_pell_compl_profile_equity %>%
            filter(dname == input[[input_names[i]]]) %>%
            select(year, transfer_out_rate) %>%
            distinct(),
          "line",
          hcaes(x = year, y = transfer_out_rate)
  ...
  output$outcomes <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_xAxis(
        categories = efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          filter(year == max(year)) %>%
      ...
      hc_add_series(
        name = "Percent Enrollment Unknown",
        efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          filter(year == max(year)) %>%
          select(cohort, pct_enrolled_unknown_pell) %>%
      ...
      hc_add_series(
        name = "Percent enrolled subsequently at another institution",
        efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          filter(year == max(year)) %>%
      ...
      hc_add_series(
        name = "Percent still enrolled at Institution",
        efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          filter(year == max(year)) %>%
      ...
      hc_add_series(
        name = "Percent receiving degree or certificate",
        efa_fsums_pell_compl_profile_equity %>%
          filter(dname == input$dname) %>%
          filter(year == max(year)) %>%
        ...
        dataLabels = list(
          enabled = TRUE,
          style = list(textOutline = FALSE, color = "black")
        )
      ) %>%
      hc_add_series(
        name = "Percent Enrollment Unknown",
      ...
      hc_yAxis(stackLabels = list(enabled = TRUE, formatter = JS(fnOut)))
  })
  ...
  output$outcomes_comp <- renderHighchart({
    ...
    highchart() %>%
      ...
      hc_add_series(
        ...
        color = "#3e6d72",
        showInLegend = FALSE,
        dataLabels = list(enabled = TRUE, style = list(textOutline = FALSE))
      ) %>%
      hc_add_series(
        name = "Percent enrolled subsequently at another institution",
        efa_fsums_pell_compl_profile_equity %>%
  ...
  output$name_icon <- renderText({
    ifelse(input$dname == "--none--",
           paste(" "),
           paste(as.character(
             icon("landmark", lib = "font-awesome")
           ), name_react()))
  })
  ...
  output$location_icon <- renderText ({
    ifelse(input$dname == "--none--",
           paste(" "),
           paste(as.character(icon(
             "map-marker", lib = "glyphicon"
           )), location_react()))
    
  })
  
  output$tribal <- renderUI({
    if (is.null(input$dname))
      return()
  ...
  output$land_grant <- renderText({
    ...
    ifelse(condition,
           paste(as.character(icon("seedling")), land_grant_react()),
           paste(land_grant_react()))
  })
  ...
  location_stuff <- renderText({
    ...
    ifelse(condition,
           paste(as.character(icon("seedling")), location_react()),
           paste(location_react()))
  })
  ...
  output$institution_graduation <- renderText({
    ...
    print({
      dname_choice() %>%  distinct(dname) %>% pull(dname)
    })
  })
        ...
        title = raw_values %>%
          ...
          mutate(
            year = paste0(
              "Year: ",
              .$year,
              "; Full-time retention rate is the percent of the fall full-time cohort from the prior year that re-enrolled at the institution in the current year."
            )
          ) %>%
          pull(year),
        placement = NULL,
  ...
  output$ihe_pell_pct <- renderUI ({
    ...
    HTML(
      glue(
        "<b><font size = 0><p style='line-height:0.25em'>Pell (%)<br></b></font></p>
  ...
  instruction_cost_cal <- reactive({
    raw_values %>%
      ...
      filter(
        variable == "instruction_current_year_total" |
          variable == "enrollment" |
          variable == "instruction_total_amount"
      ) %>%
      filter(year == max(year)) %>%
  ...
  coordinate_react_comp <- reactive({
    fsums %>%
      ...
      filter(dname %in% c(input$dname, input$dname_comp)) %>%
  ...
  drilldown_react_df <- reactive({
    fsums %>%
      ...
      mutate_at(
        ...
        str_replace_all,
        pattern = "Assets",
        replacement = "Assets - A resource of economic value"
      ) %>%
      mutate_at(
        c("category_definition"),
        str_replace_all,
        pattern = "Debt",
        replacement = "Debt - An amount owed for funds borrowed"
      ) %>%
      mutate_at(
        c("category_definition"),
        str_replace_all,
        pattern = "Expenses",
        replacement = "Expenses - The reduction in value of an asset as it is used to generate revenue"
      ) %>%
      mutate_at(
        c("category_definition"),
        str_replace_all,
        pattern = "Revenue",
        replacement = "Revenue - An increase in assets or decrease in liabilities caused by the provision of services or products to customers"
      ) %>%
      mutate_at(
        c("category_definition"),
        str_replace_all,
      ...
      arrange(factor(
        category,
        levels = c("Revenue", "Expenses", "Assets", "Debt", "Students")
      ))
  ...
  drilldown_react_plot <- reactive({
    ...
    title_with_score <- dname_choice() %>%
      ...
      mutate(
        title = glue(
          "{institution_entity_name} Index Score: {round(total, 2)} ({pct_rank} Percentile)"
        )
      ) %>%
      pull(title)
    ...
    highchart() %>%
      ...
      hc_yAxis(
        ...
        title = list(
          text = "FHI Sub-Items",
          style = list(fontSize = "3vh", fontWeight =
                         "bold")
        )
      ) %>%
      hc_xAxis(
        ...
        title = list(
          text = "FHI Score",
          style = list(fontSize = "3vh", fontWeight = "bold")
        )
      ) %>%
      hc_chart(inverted = T) %>%
      ...
      hc_plotOptions(series = list(
        ...
        events = list(
          mouseOver = JS(
            "function() { if(this.options.color !== 'red') {this.update({color: 'red'})} }"
          ),
          mouseOut = JS(
            "function() { if(this.options.color === 'red') {this.update({color: '#ddd'})} }"
          )
        )
      ))
  ...
  raw_features_data <- eventReactive(input$canvasClicked[2], {
    ...
    rfd <- raw_values %>%
      ...
      left_join(
        current_feature() %>% select(variable, fraction_part, sequence, var_title, description)
      ) %>%
  ...
  text_formula <- eventReactive(input$canvasClicked[2], {
    ...
    nice_var_features <- current_feature() %>%
      ...
      distinct() %>%
      mutate(
        numerator_operation = case_when(
          numerator_operation == "plus" ~ "+",
      ...
      mutate(formula = case_when(is.na(formula) ~ as.character(var_title), TRUE ~
                                   formula)) %>%
  ...
  observeEvent(input$canvasClicked[2], {
    outputText <<- paste0(
      "You clicked on series ",
      input$canvasClicked[1],
      " and the bar you clicked was from category ",
  ...
  output$formula <- renderUI({
    withMathJax(helpText(
      paste(
        input$canvasClicked[2],
        "$$\\frac{",
        text_formula() %>% filter(fraction_part == "numerator") %>% pull(formula),
        "}",
        '{',
  ...
  enroll_grad_column <- reactive({
    efa_fsums_pell_compl_profile_equity %>%
      ...
      filter(dname == input$dname) %>%
      group_by(race) %>%
      mutate(mean_enrollment = mean(enrollment_pct_per_inst_year_race) *
               100) %>%
      mutate(mean_graduation = mean(completers_pct_per_inst_year_race) *
               100) %>%
      ...
      hc_yAxis(
        ...
        plotBands = list(
          list(
            from = parallel_chart() %>%
              filter(race == "AIAN" &
                       var_names == "pop_pct_per_state_year_race") %>%
              pull(pct_count_values),
            to = parallel_chart() %>%
              filter(
                race == "AIAN" & var_names == "completers_pct_per_inst_year_race"
              ) %>%
              pull(pct_count_values),
          ...
          list(
            ...
            to = parallel_chart() %>%
              filter(
                race == "Asian" &
                  var_names == "completers_pct_per_inst_year_race"
              ) %>%
              pull(pct_count_values),
          ...
          list(
            from = parallel_chart() %>%
              filter(
                race == "Hispanic" & var_names == "pop_pct_per_state_year_race"
              ) %>%
              pull(pct_count_values),
            to = parallel_chart() %>%
              filter(
          ...
          list(
            ...
            to = parallel_chart() %>%
              filter(
                race == "NHPI" & var_names == "completers_pct_per_inst_year_race"
              ) %>%
              pull(pct_count_values),
  ...
  output$equityTable <- renderReactable({
    ...
    data1 <- efa_fsums_pell_compl_profile_equity %>%
      ...
      mutate(count_type = case_when(
        str_detect(var_names, "_count_") ~ "count",
      ...
      group_by(var_names, race) %>%
      mutate(mean_count = mean(count, na.rm = T),
             mean_pct = mean(pct, na.rm = T)) %>%
      ...
      mutate(
        var_names = case_when(
          var_names == 'completers_per_inst_year_race' ~ "Completers",
          var_names == 'enrollment_per_inst_year_race' ~
            "Enrollment",
          var_names == 'instructor_per_inst_year_race' ~
            "Instructors",
          var_names == 'pop_per_natl_year_race' ~
            "Nation",
          var_names == 'pop_per_state_year_race' ~
            "State"
        )
      )
